...
load_dotenv()
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "AIzaSyCvGROaAsFA2mMeODg94wQjh8zoqraUwcc")
...
intention_detector = IntentionDetector(genai_client=genai)
...
def create_embeddings_fallback():
    ...
    try:
        ...
        # Preparar textos
        for idx, doc in enumerate(documents):
            # Construir texto usando solo los campos solicitados
            parts = []
...
# ============================================================================
# FUNCIÓN DE NORMALIZACIÓN CON GEMINI
# ============================================================================

def normalize_with_gemini(text: str) -> str:
    """
    Normaliza texto usando Gemini 2.0 Flash para corregir errores de transcripción.
    
    Corrige:
    - Palabras partidas: "ante c edentes" → "antecedentes"
    - Espacios múltiples: "solicitan  ante" → "solicitan ante"
    ...
    Returns:
        Texto normalizado o texto original si falla
    """
    if not text or not text.strip():
        return text
    
    # Si Gemini no está disponible, hacer normalización básica
    if not GENAI_AVAILABLE:
        print("⚠️ Gemini no disponible, usando normalización básica")
        return text.strip()
    
    try:
        prompt = f"""Eres un corrector de texto especializado en normalizar transcripciones de voz a texto.

Tu tarea es corregir SOLO errores de transcripción, manteniendo el significado original.

REGLAS ESTRICTAS:
1. Corrige palabras partidas por espacios incorrectos (ej: "ante c edentes" → "antecedentes")
2. Elimina espacios múltiples innecesarios
3. Corrige errores ortográficos obvios de transcripción
4. NO cambies el significado ni agregues información
5. NO traduzcas ni interpretes
6. Mantén nombres propios, fechas y números exactamente como están
...
Texto corregido:"""
        ...
        response = model.generate_content(
            prompt,
            generation_config={
                'temperature': 0.1,  # Muy baja para ser conservador
        ...
        # Validación: no debe ser vacío ni muy diferente en longitud
        if not normalized:
            print("⚠️ Gemini devolvió texto vacío, usando original")
            return text.strip()
        
        # Si el cambio es muy drástico (>50% de diferencia), usar original
        len_diff = abs(len(normalized) - len(text)) / len(text)
        if len_diff > 0.5:
            print(f"⚠️ Cambio muy drástico ({len_diff:.1%}), usando texto original")
            return text.strip()
        
        # Registrar el cambio si lo hubo
        if normalized != text.strip():
            print(f"✨ Gemini normalizó:")
            print(f"   '{text}' → '{normalized}'")
        
        return normalized
        
    except Exception as e:
        print(f"❌ Error en normalización con Gemini: {e}")
        return text.strip()
...
def chat():
    ...
    try:
        ...
        # ============================================================
        # NORMALIZACIÓN CON GEMINI - PASO CRÍTICO
        # ============================================================
        print(f"🔧 Iniciando normalización con Gemini...")
        normalized_query = normalize_with_gemini(original_query)
        ...
        if normalized_query != original_query:
            ...
            print(f"   Normalizado: '{normalized_query}'")
        else:
            print(f"ℹ️  Sin cambios en la normalización")
        
        print(f"🔎 Query final procesada: '{query}'")
        print(f"🆔 Session ID: {session_id}")
        event_bus.publish('chat.received', {'query': query, 'original': original_query})
        ...
        if conversation_type in ['greeting', 'farewell', 'gratitude', 'help', 'smalltalk']:
            ...
            if response_text:
                ...
                return jsonify({
                    ...
                    'original_query': original_query,
                    'normalized_query': normalized_query,
                    'was_normalized': (original_query != normalized_query)
                })
        ...
        if is_follow_up:
            ...
            # IMPORTANTE: Usar query ya normalizada para evitar doble normalización
            # Verificar PRIMERO si hay búsqueda explícita (antes de limpiar saludos)
            has_search = intention_detector.has_explicit_search(query)
            print(f"🔍 ¿Búsqueda explícita? {has_search}")
            
            # Si no hay búsqueda explícita, verificar si tiene contenido sustantivo
            # Esto maneja casos donde el usuario repite la misma búsqueda
            if not has_search:
                # Contar palabras sustantivas (no son palabras de parada emocionales)
                words = query.lower().split()
                # Palabras que indican solo expresión emocional, no búsqueda
                stop_words = {'no', 'me', 'sirve', 'gracias', 'perfecto', 'excelente', 
                             'bien', 'mal', 'ok', 'vale', 'genial', 'si', 'sí'}
                substantive_words = [w for w in words if w not in stop_words and len(w) > 2]
                
                # Si tiene más de 2 palabras sustantivas, es probablemente una búsqueda
                if len(substantive_words) >= 3:
                    has_search = True
                    print(f"✅ Detectado contenido sustantivo: {len(substantive_words)} palabras → Tratando como búsqueda")
            
            # Si NO hay búsqueda, detectar intención (satisfecho/insatisfecho)
            if not has_search:
                ...
                if intention == 'satisfied':
                    response_text = "¡Excelente! 😊 Me alegra haber encontrado lo que buscabas.\n\n¿Hay algo más que quieras explorar en el Archivo Patrimonial?"
                    response_html = markdown.markdown(response_text)
                    event_bus.publish('response.generated', {'chars': len(response_text), 'docs': 0})
                ...
                # Usuario insatisfecho SIN información adicional
                if intention == 'unsatisfied':
                    response_text = """❓ Entiendo que no encontraste lo que buscabas. 
...
Cuéntame más y haré una búsqueda más dirigida. 📚"""
                    ...
                    return jsonify({
                        ...
                        'documents': [],
                        'embeddings_ready': embeddings_ready,
                        'conversation_type': 'follow_up_unsatisfied',
                        'session_id': session_id,
                        'original_query': original_query,
                        'normalized_query': normalized_query,
                        'was_normalized': (original_query != normalized_query)
            ...
            # Hay búsqueda explícita - continuar con búsqueda
            print(f"🔍 Continuando con búsqueda de seguimiento: '{query}'")

        # ============ FLUJO NORMAL (primera búsqueda o búsqueda refinada) ============
        print(f"🔍 Realizando búsqueda de documentos con query normalizada...")
        ...
        # Registrar búsqueda en la sesión (con la query normalizada)
        ...
        return jsonify({
            ...
            'normalized_query': normalized_query if 'normalized_query' in locals() else None
        }), 500
...
def normalize_text():
    """
    Endpoint para normalizar texto usando Gemini 2.0 Flash
    
    Corrige errores comunes de transcripción de voz a texto (speech-to-text):
    - Palabras partidas (ej: 'ant ecedentes' → 'antecedentes')
    - Espacios múltiples innecesarios
    - Errores ortográficos leves
    
    Body: { "text": "texto a normalizar" }
    """
    # Manejar preflight CORS
    if request.method == 'OPTIONS':
        response = jsonify({'status': 'ok'})
    ...
    try:
        ...
        # Usar IntentionDetector para normalizar
        normalized_text = intention_detector.normalize_text(original_text)
        
        # Determinar si hubo cambios
        was_normalized = (normalized_text != original_text)
        ...
        print(f"❌ Error en /api/normalize: {e}")
        traceback.print_exc()
        return jsonify({
            'success': False,
            'error': 'Error normalizando el texto',
            'details': str(e)
        }), 500
...
if __name__ == '__main__':
    # Soporte para puerto dinámico en Koyeb
