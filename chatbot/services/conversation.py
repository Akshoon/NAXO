...
class IntentionDetector(IntentionStrategy):
    ...
    def __init__(self, patterns: Optional[Dict[str, List[str]]] = None, genai_client: Any = None):
        """Inyección de dependencias (DIP)
        
        Args:
            patterns: Dict con patrones regex personalizados
            genai_client: Cliente de Google GenAI para normalización avanzada
        """
    ...
    def normalize_text(self, message: str) -> str:
        """Normaliza el texto corrigiendo errores comunes de transcripción
        
        Intenta usar Gemini 2.0 Flash para una corrección inteligente.
        Si falla o no está disponible, usa reglas heurísticas (regex).
        ...
        # 1. Intentar con Gemini 2.0 Flash (con prompt mejorado y más conservador)
        if self.genai_client:
            try:
                model = self.genai_client.GenerativeModel("gemini-2.0-flash")
                prompt = (
                    "Eres un corrector de transcripciones de voz a texto. Tu tarea es corregir ÚNICAMENTE errores evidentes.\n\n"
                    "REGLAS IMPORTANTES:\n"
                    "1. SOLO corrige palabras que claramente están partidas por espacios incorrectos (ej: 'ant ecedentes' → 'antecedentes')\n"
                    "2. SOLO elimina espacios múltiples innecesarios (ej: 'texto   con   espacios' → 'texto con espacios')\n"
                    "3. NO cambies palabras que ya están correctamente escritas\n"
                    "4. NO agregues, quites o modifiques palabras que no tienen errores\n"
                    "5. PRESERVA el significado y estructura original\n"
                    "6. Si NO hay errores evidentes, devuelve el texto EXACTAMENTE igual\n\n"
                    "Ejemplos de correcciones válidas:\n"
                    "- 'ant ecedentes' → 'antecedentes' (palabra partida)\n"
                    "- 'doc umentos' → 'documentos' (palabra partida)\n"
                    "- 'texto   con   muchos    espacios' → 'texto con muchos espacios' (espacios múltiples)\n\n"
                    "Ejemplos de textos que NO debes cambiar:\n"
                    "- 'antecedentes' → 'antecedentes' (ya está correcto)\n"
                    "- 'solicitan antecedentes' → 'solicitan antecedentes' (ya está correcto)\n\n"
                    "IMPORTANTE: Devuelve ÚNICAMENTE el texto corregido, sin explicaciones ni comentarios.\n\n"
                    f"Texto a revisar: {message}"
                )
                response = model.generate_content(prompt)
                if response and response.text:
                    gemini_result = response.text.strip()
                    
                    # VALIDACIÓN: Verificar que Gemini no haya empeorado el texto
                    # Si el texto ya no tenía errores evidentes de palabras partidas, usar el original
                    if self._has_split_words(original_message) or self._has_multiple_spaces(original_message):
                        # Había errores potenciales, usar resultado de Gemini
                        return gemini_result
                    else:
                        # No había errores evidentes, mantener original
                        # PERO verificar que Gemini no haya cambiado el texto innecesariamente
                        if gemini_result.lower().replace(" ", "") == original_message.lower().replace(" ", ""):
                            # Solo cambios de espaciado, aceptar
                            return gemini_result
                        else:
                            # Cambios más significativos en texto que ya estaba bien, rechazar
                            print(f"ADVERTENCIA: Gemini modifico texto sin errores evidentes. Usando original.")
                            return original_message
                        
            except Exception as e:
                print(f"ADVERTENCIA: Error normalizando con Gemini: {e}. Usando fallback.")
        
        # 2. Fallback: Heurísticas Regex (Código original)
        
        # Diccionario de correcciones comunes de speech-to-text
        common_fixes = {
            r'\bant\s+ecedentes\b': 'antecedentes',
            r'\bdoc\s+umentos\b': 'documentos',
            r'\binform\s+ación\b': 'información',
            r'\bfotograf\s+ías\b': 'fotografías',
            r'\blicenc\s+ia\b': 'licencia',
            r'\bméd\s+ica\b': 'médica',
            r'\bsolic\s+itan\b': 'solicitan',
            r'\bdictad\s+ura\b': 'dictadura',
            r'\bgob\s+ierno\b': 'gobierno',
            r'\bderech\s+os\b': 'derechos',
            r'\bhuman\s+os\b': 'humanos',
        }
        
        normalized = message
        
        # Aplicar correcciones conocidas (case-insensitive)
        for pattern, replacement in common_fixes.items():
            normalized = re.sub(pattern, replacement, normalized, flags=re.IGNORECASE)
        
        # Eliminar espacios múltiples (reemplazar 2+ espacios por 1)
    ...
    def _has_split_words(self, text: str) -> bool:
        """Detecta si el texto tiene palabras partidas por espacios incorrectos"""
        # Patrones comunes de palabras partidas
        split_patterns = [
            r'\b\w+\s+\w{1,4}\b',  # Palabra seguida de fragmento corto (ej: "ant ecedentes")
        ]
        
        for pattern in split_patterns:
            if re.search(pattern, text):
                # Verificar si podría ser una palabra partida
                matches = re.findall(pattern, text)
                for match in matches:
                    # Si el fragmento tiene menos de 5 caracteres después del espacio, podría ser partición
                    parts = match.split()
    ...
    def _has_multiple_spaces(self, text: str) -> bool:
        """Detecta si el texto tiene múltiples espacios consecutivos"""
    ...
    def remove_greetings(self, message: str) -> str:
        ...
        Normaliza el texto primero para corregir errores de transcripción,
        luego elimina saludos comunes.
        
        Ejemplo: 
        - "Hola, busco guerra del pacífico" → "busco guerra del pacífico"
        - "Buenos días, ant ecedentes médicos" → "antecedentes médicos"
        
        Returns: Mensaje sin saludos al inicio y con texto normalizado
        """
        # PASO 1: Normalizar texto para corregir errores de transcripción
        normalized = self.normalize_text(message)
        
        # PASO 2: Convertir a minúsculas para procesamiento
        ...
        # Si después de limpiar queda vacío, retornar original normalizado
