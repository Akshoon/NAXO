...
<div class="uah-banner">
    A través de este portal usted puede buscar documentos, imágenes, u otros documentos digitalizados, pudiendo buscar
    aquella información que es de su interés.
</div>
...
<style>
    ...
    .folder-btn {
        ...
        align-items: center;
        justify-content: center;
        background: linear-gradient(145deg, var(--chatbot-white) 0%, #f8fafc 100%);
        border: 2px solid var(--chatbot-border);
        border-radius: 16px;
        padding: 16px 10px;
        cursor: pointer;
    ...
    .modal-header {
        background: linear-gradient(135deg, var(--chatbot-accent) 0%, #d4b94c 100%);
        color: var(--chatbot-primary);
        padding: 16px 20px;
        display: flex;
        justify-content: space-between;
...
<div id="chatbot-container">
    ...
    <div id="chatbot-messages">
        <div class="welcome-container">
            ...
            <div class="folder-grid">
                <button class="folder-btn" onclick="openCategoriesWithTab('materias')"><span
                        class="folder-icon">📚</span><span class="folder-label">Materias</span><span
                        class="folder-count">2644</span></button>
                <button class="folder-btn" onclick="openCategoriesWithTab('autores')"><span
                        class="folder-icon">👤</span><span class="folder-label">Autores</span><span
                        class="folder-count">3191</span></button>
                <button class="folder-btn" onclick="openCategoriesWithTab('lugares')"><span
                        class="folder-icon">📍</span><span class="folder-label">Lugares</span><span
                        class="folder-count">561</span></button>
            </div>
            <p style="font-size:11px; color:var(--chatbot-text-muted); margin-top:12px;">O escribe tu búsqueda abajo ⬇️
            </p>
        </div>
    </div>
    <div id="categories-modal">
        <div class="modal-header">
            <h4>📂 Explorar Categorías</h4><button class="modal-close" id="close-categories">✕</button>
        </div>
        <div class="category-tabs"><button class="category-tab active" data-tab="materias">📚 Materias</button><button
                class="category-tab" data-tab="autores">👤 Autores</button><button class="category-tab"
                data-tab="lugares">📍 Lugares</button></div>
        <div class="category-search"><input type="text" id="category-search-input" placeholder="🔍 Buscar..."
    ...
    <div id="chatbot-input">
        ...
        <button id="chatbot-send"><svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        async function searchByCategory(type, name) {
            ...
            try {
                const r = await fetch(API_BASE + '/search-by-category', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ category_type: type, category_name: name }) }); const data = await r.json(); loading.remove();
